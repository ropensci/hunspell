...
Hunspell uses a special dictionary format that defines which characters, words and conjugations are valid in a given language. The examples below use the (default) `"en_US"` dictionary. However each function can be used in another language by setting a custom dictionary in the `dict` parameter. See the section on [dictionaries](#dictionaries) below.
...
If that didn't work, double check that the files were installed in the system directory, usually `/usr/share/myspell` or `/usr/share/hunspell`.
